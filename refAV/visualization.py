...
#     with open('results/log_id2idx_mapping.json', 'rb') as file:
#         log_id_to_start_index = json.load(file)

#     i = int(log_id_to_start_index[log_id])
#     datum = dataloader[i]
...
#     anno_df = pd.read_feather(annotations_path)
#     # import pdb;pdb.set_trace()
#     while data_info[i]['uuid'].split('/')[0] == log_id:
...
#             # datum = dataloader[i]
#             # import pdb;pdb.set_trace()
#             timestamp = int(data_info[i]['uuid'].split('/')[1])
#             timestamp_df = df[df['timestamp_ns'] == timestamp]
#             referred_df = timestamp_df[timestamp_df['mining_category'] =='REFERRED_OBJECT']
...
#     annotation_dir=Path('output/tracker_predictions/Le3DE2E_tracking/val')

#     visualize_rgb(dataset_dir, feather_path, output_dir, log_id, description="Vehicle making left turn through ego-vehicle's path while it is raining",annotation_dir=annotation_dir)
